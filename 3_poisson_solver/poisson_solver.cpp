...
// int read_input(double &x1_min, double &x1_max, double &x2_min, double
// &x2_max,
//                int &nx, int &ny) {
int read_input(std::array<double, 2> &x1_range, std::array<double, 2> &x2_range,
               std::array<int, 2> &n) {
...
prepare_mesh(std::array<double, 2> &x1_range, std::array<double, 2> &x2_range,
             std::array<int, 2> n) {
  ...
  parallel_for(
      MDRangePolicy({0, 0}, {u.extent(0), u.extent(1)}), [=](int i, int j) {
        if (i > 0 && i < u.extent(0) - 1 && j > 0 && j < u.extent(1) - 1) {
          u(i, j) = (u_old(i - 1, j) + u_old(i + 1, j) + u_old(i, j - 1) +
                     u_old(i, j + 1)) /
                    4;
        }
      });
...
int main(int argc, char **argv) {
  ...
  printf("x1_range=[%f, %f], x2_range=[%f, %f], nx, ny=%d, %d\n", x1_range[0],
         x1_range[1], x2_range[0], x2_range[1], n[0], n[1]);
